...
IMAGE = Coat(img_url).rsize(fx=0.5,fy=0.5).blur_gauss((5,5))#.to_color('BGR2GRAY')

#IMAGE = Coat(img_url).to_color('BGR2GRAY').rsize(fx=.5,fy=0.5).astype(np.float32)/255.
...
def segmentation(img,size,clusters=10,dist=False):
    ...
    for x in tqdm(range(IMAGE.shape[0]-s+1)):
        for y in range(IMAGE.shape[1]-s+1):
            ...
            counter +=1
    return Coat(a/K)
img1 = segmentation(IMAGE,3,clusters=7,dist=True)
img2 = segmentation(IMAGE,3,clusters=7,dist=False)
